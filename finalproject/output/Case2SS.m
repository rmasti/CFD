...
%% Mesh 1
x = load('Case_2_Mesh_1_Flux_1_order_2/xc-0.txt');
y = load('Case_2_Mesh_1_Flux_1_order_2/yc-0.txt');
ni = length(x(1,:)); 
nj = length(x(:,1));

% plot(x(21,1),y(21,1),'*')
plot(x(1,1),y(1,1),'r*') ; %lower wall begin
hold on
plot(x(52,1),y(52,1),'r*'); % lowver wall end
% upper wall
plot(x(21,16),y(21,16),'b*'); %begin
plot(x(end, end), y(end, end), 'b*')%end
% Inlet wall
plot(x(1,16),y(1,16),'k*'); %begin
plot(x(20, 16), y(20, 16), 'k*')%end
% oulet wall
plot(x(52,1),y(52,1),'g*'); %begin
plot(x(52, 16), y(52, 16), 'g*')%end
% symmetric wall
plot(x(1,1),y(1,1),'c*'); %begin
plot(x(1, 16), y(1, 16), 'c*')%end


mms = zeros(nj, ni, NEQ);
num = zeros(nj, ni, NEQ);
% conserved vars
% rhou9 = load('Case_1_Mesh_1_Flux_1_order_2/rhou-98.txt');
% rhov9 = load('Case_1_Mesh_1_Flux_1_order_2/rhov-98.txt');
% rhoe9 = load('Case_1_Mesh_1_Flux_1_order_2/rhoe-98.txt');
% primvar mms
temp = load('Case_2_Mesh_1_Flux_1_order_2/rho-79.txt');

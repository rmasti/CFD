...
int main()
{
  ...
  consts.A_t = A_x(0.0);//should be m^2 - for the exact solution
  ...
  // 2 for van albada 
  // 3 for Ospre
  // 4 for Monotized Central
  // 5 for MinMod
  // 6 for SuperBee
  consts.limiter = 1; 
  //////////////////////// START of SIMULATIONS ////////////////////////////
  //This main file is kept purposely really short and it just tells these two files to run 
  //their respective sims (exact solution, supersonic outflow, and subsonic outflow)
...
void quasi1Dnozzle(constants C)
{
  ...
  for (int i = 0 ; i < neq ; i++)
  {
    ...
    V_L[i].resize(1,N+1);
    V_R[i].resize(1,N+1);
    Psi_Neg[i].resize(1,number_of_cells+1); //Also can have negative values
    Psi_Pos[i].resize(1,number_of_cells+1); //Every interface can have a psi value*****
  }
  ...
  for(int n= 0 ;  n < nmax; n++)
  {
    ...
    else
    {
      ...
      // Apply the flux limiter freezing if need be
      if (n < nmax/100)
        compute_upwind_VLR(V_L, V_R, Psi_Pos, Psi_Neg, V, C, false); //freeze=false
      else
        compute_upwind_VLR(V_L, V_R, Psi_Pos, Psi_Neg, V, C, true); //freeze=true
    ...
    if (n == 3) // Create relative residual
    {
    ...
    if (n % nwriteout == 0)
    {
      ...
      //output_array("U1_iter", U[rhoid], n); // debug mode
      //output_array("U2_iter", U[rhouid], n);
      //output_array("U4_iter", U[rhoetid], n);
    }
  }
}
